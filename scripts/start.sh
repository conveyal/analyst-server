...
ANALYST_MEM=`echo "$TOTAL_MEM - (5500 * 1024)" | bc`
...
java -Xmx3G -Dlogback.configurationFile=/etc/broker-logentries.xml -cp /opt/otp/analyst-server.jar com.conveyal.r5.analyst.broker.BrokerMain /etc/broker.conf > /home/ubuntu/broker.log < /dev/null 2>&1 &
echo $! > /tmp/BROKER_PID
