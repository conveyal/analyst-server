...
cd /ebs/scratch
java -Xmx7G -Dlogback.configurationFile=/etc/analyst-logentries.xml -jar /opt/otp/analyst-server.jar /etc/analyst.conf > /home/ubuntu/analyst.log < /dev/null 2>&1 &
...
java -Xmx7G -Dlogback.configurationFile=/etc/broker-logentries.xml -cp /opt/otp/analyst-server.jar com.conveyal.r5.analyst.broker.BrokerMain /etc/broker.conf > /home/ubuntu/broker.log < /dev/null 2>&1 &
echo $! > /tmp/BROKER_PID
